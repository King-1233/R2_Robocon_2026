  ...
  for (;;)
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      RobStrideMotionControl(&m->Rs_motor,
		...
		vTaskDelay(2);
  //  RobStrideMotionControl(&Claw.Rs_motor,Claw.Rs_motor.motor_id,Claw.exp_torque,Claw.exp_rad + Claw.pos_offset,Claw.exp_omega,kp,kd);
...
void claw_Prepare_task(uint8_t  *num)
{
  ...
  {
  case 1:	
    LiftSystem.lift_cmd[0].target_height = 0.40f;
    LiftSystem.lift_cmd[2].target_height = 0.40f;
    test_triggered_test_All = 1;
    Claw.exp_rad = 1.5f;
    break;
  case 2:
    LiftSystem.lift_cmd[0].target_height = 0.367f;
    LiftSystem.lift_cmd[2].target_height = 0.367f;
    test_triggered_test_All = 1;
  ...
  case 5:
    ...
    break;
  case 6:
	  LiftSystem.lift_cmd[0].target_height = 0.11f;
    LiftSystem.lift_cmd[1].target_height = -0.03f;
    LiftSystem.lift_cmd[2].target_height = 0.11f;
     test_triggered_test_All= 1;
    ...
    Motor1.value = 999.0f;
		LiftSystem.lift_cmd[0].target_height = 0;
    LiftSystem.lift_cmd[2].target_height = 0;
	  test_triggered_test_All = 1;
   break;
  default:
    break;
...
void Check_Case4_Completion(void)
{
    if (wait_case4_finish)
    {
        float target_rad_0 = distance_to_motor_rad(0.42f, LiftSystem.motors[0].Rs_motor.motor_id);
        if (IsMotorAtTarget(&LiftSystem.motors[0], target_rad_0))
        {
            Send_TransMotor_State(2); // 动作完成发 2
            wait_case4_finish = 0;    // 清除标志位
        }
    }
}