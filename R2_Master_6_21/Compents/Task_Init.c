...
void Uart_Tx(void *pvParameters)
{
  ...
  while(1)
  {
		...
		else if (LiftSystem.work_mode == ROMOTE_MODE)//||LiftSystem.work_mode == LIFT_MODE_CLIMB_UP || LiftSystem.work_mode == LIFT_MODE_CLIMB_DOWN)
		{
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t size)
{
	...
	if (huart->Instance == USART2)
	{
		if (size == 9)
		{
			if (usart2_dma_buff[0] == 0xAC && usart2_dma_buff[8] == 0xCA)
			{
				LiftSystem.sensor_front[0].distance = ((uint32_t)usart2_dma_buff[1] << 24) |
                                               ((uint32_t)usart2_dma_buff[2] << 16)|
                                               ((uint32_t)usart2_dma_buff[3] << 8)|
                                               (uint32_t)usart2_dma_buff[4];
				LiftSystem.sensor_front[0].possibility = usart2_dma_buff[5];
				DTdata.spi1 = (usart2_dma_buff[6] << 8) | usart2_dma_buff[7];
			}
		}
		HAL_UARTEx_ReceiveToIdle_DMA(&huart2, usart2_dma_buff, sizeof(usart2_dma_buff));
		__HAL_DMA_DISABLE_IT(huart2.hdmarx, DMA_IT_HT);
	...
	if (huart->Instance == USART3)
	{
            HAL_UART_DMAStop(huart);
				VL53_Final_DataProcess(usart3_dma_buff,sizeof(usart3_dma_buff),&LiftSystem.sensor_rear);
			HAL_UARTEx_ReceiveToIdle_DMA(&huart3, usart3_dma_buff, sizeof(usart3_dma_buff));
			__HAL_DMA_DISABLE_IT(huart3.hdmarx, DMA_IT_HT);
	...
	if (huart->Instance == USART6)
	{
		if (size == 5)
		{
			if (usart6_dma_buff[0] == 0xBC && usart6_dma_buff[5] == 0xCB)
			{
				if (usart6_dma_buff[4] == 1)
				{
					LiftSystem.sensor_front[1].distance = (usart6_dma_buff[1] << 8) | usart6_dma_buff[2];
					LiftSystem.sensor_front[1].possibility = usart6_dma_buff[3];
				}
			}
			__HAL_DMA_DISABLE_IT(huart6.hdmarx, DMA_IT_HT);
		}
